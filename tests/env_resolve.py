...
Vendored verbatim from on-the-record's `gates/test_env_resolve.py`
(docs/specs/test-env-resolution.md, issue #551). Renamed to avoid the
`test_` prefix so bare `pytest` collection of this repo does not pick it
up as a test file. Logic and constants unchanged.

  python3 tests/env_resolve.py <candidate1> <candidate2> ...
"""
